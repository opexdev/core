package co.nilin.mixchange.matching.core.engine

import co.nilin.mixchange.matching.core.eventh.EventDispatcher
import co.nilin.mixchange.matching.core.eventh.events.*
import co.nilin.mixchange.matching.core.inout.*
import co.nilin.mixchange.matching.core.model.*
...
class SimpleOrderBook(val pair: Pair, var replayMode: Boolean) : OrderBook {
    ...
    override fun handleNewOrderCommand(orderCommand: OrderCreateCommand): Order? {
        val order = when (orderCommand.matchConstraint) {
            MatchConstraint.GTC -> {
                ...
                val order = SimpleOrder(orderCounter.incrementAndGet(), orderCommand.ouid, orderCommand.uuid, orderCommand.price, orderCommand.quantity, orderCommand.matchConstraint, orderCommand.orderType, orderCommand.direction, 0, null, null, null)
                if (!replayMode) {
    ...
    override fun handleEditCommand(orderCommand: OrderEditCommand): Order? {
        ...
        val newOrder = SimpleOrder(order.id, orderCommand.ouid, orderCommand.uuid,orderCommand.price, orderCommand.quantity, order.matchConstraint, order.orderType, order.direction, order.filledQuantity, null, null, null)
    ...
    fun putGtcInQueue(order: SimpleOrder,
        ...
        if (bucket != null) {
            ...
            bucket = Bucket(order.price, order.remainedQuantity(), 1, order)
            order.bucket = bucket
    ...
    fun rebuild(persistentOrderBook: PersistentOrderBook) {
        persistentOrderBook.orders?.map { order ->
           SimpleOrder(order.id, order.ouid, order.uuid, order.price, order.quantity, order.matchConstraint, order.orderType, order.direction, order.filledQuantity, null, null, null)
