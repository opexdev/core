...
Security: Bearer user-token required. Requires authenticated user JWT.""",
    ...
    suspend fun getProfile(@Parameter(hidden = true)
                           @CurrentSecurityContext securityContext: SecurityContext): ProfileResponse {
...
Security: Bearer user-token required. Requires authenticated user JWT.
Validation: Exactly one of `mobile` or `email` must be provided. Providing both or neither is invalid.
""",

        security = [SecurityRequirement(name = "bearerAuth")],
        responses = [
            ApiResponse(responseCode = "200", description = "Successful response.", content = [Content(mediaType = "application/json", schema = Schema(implementation = TempOtpResponse::class))]),
            ApiResponse(responseCode = "401", description = "Unauthorized. Bearer token is missing, invalid, or expired. No response body.", content = [Content()])
        ]
    )
...
Security: Bearer user-token required. Requires authenticated user JWT.""",
        security = [SecurityRequirement(name = "bearerAuth")],
        responses = [
            ApiResponse(responseCode = "200", description = "Successful response.", content = [Content(mediaType = "application/json", schema = Schema(implementation = ProfileApprovalRequestUserResponse::class))]),
            ApiResponse(responseCode = "401", description = "Unauthorized. Bearer token is missing, invalid, or expired. No response body.", content = [Content()])
        ]
    )
    suspend fun getApprovalRequest(@Parameter(hidden = true)
                                   @CurrentSecurityContext securityContext: SecurityContext): ProfileApprovalRequestUserResponse {
