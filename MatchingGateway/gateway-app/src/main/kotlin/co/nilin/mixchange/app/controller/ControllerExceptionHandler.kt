...
class ControllerExceptionHandler {
    ...
    fun handle(ex: NotAllowedToSubmitOrderException): ResponseEntity<ErrorResponse> {
        ...
        val ret = ResponseEntity.status(500).body(ErrorResponse(Date()
                , -1
    ...
    fun handle(ex: WebClientResponseException): ResponseEntity<ErrorResponse> {
        logger.error("Trace Error {}", ex )
        try {
            val body = objectMapper.readValue(ex.responseBodyAsByteArray.toString(StandardCharsets.UTF_8), WebClientErrorResponse::class.java)
            val ret = ResponseEntity.status(body.status ?: ex.rawStatusCode).body(ErrorResponse(Date()
            ...
            val ret = ResponseEntity.status(ex.statusCode).body(ErrorResponse(Date()
                    , ex.rawStatusCode
                    , ex::class.qualifiedName ?: ""
