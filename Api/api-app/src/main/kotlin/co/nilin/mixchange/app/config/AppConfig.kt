...
import co.nilin.mixchange.port.api.kafka.consumer.OrderKafkaListener
...
import co.nilin.mixchange.port.trade.spi.RichTradeListener
...
class AppConfig {
    ...
    @Bean
    fun apiListener(
            richOrderPersister: OrderPersister, richTradePersister: TradePersister
    ): ApiListenerImpl {
    ...
    fun configureListeners(
            orderKafkaListener: OrderKafkaListener, tradeKafkaListener: TradeKafkaListener, appListener: ApiListenerImpl
    ) {
    ...
    class ApiListenerImpl(
            val richOrderPersister: OrderPersister, val richTradePersister: TradePersister
        ...
        override fun onTrade(
                trade: co.nilin.mixchange.accountant.core.inout.RichTrade,
                partition: Int,
                offset: Long,
                timestamp: Long
        ) {
        ...
        override fun onOrder(order: RichOrder, partition: Int, offset: Long, timestamp: Long) {
            runBlocking(AppDispatchers.kafkaExecutor) {
