...
interface UserWithdrawVolumePersister {
    suspend fun update(
        userId: String, currency: String, amount: BigDecimal, date: LocalDateTime, withdrawStatus: WithdrawStatus
    )

    suspend fun getTotalValueByUserAndDateAfter(uuid: String, startDate: LocalDateTime): BigDecimal

    suspend fun getLastDaysWithdraw(userId: String, startDate: LocalDate?, quatCurrency: String?, lastDays: Long = 31): List<DailyAmount>
}